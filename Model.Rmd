...
```{r}
pm2_n <- pm2 %>% 
  # Velg først variablene så groupd mht. aar_d (date objektet)
  #group_by(aar) %>%
  # lagt til aar_d som vi trenger nedenfor
...
```{r}
# Må bruke argumentet i funksjonen. Her vil R lete etter et objekt
# i global som heter pm2. Finner det og bruker dette samme hva dere sender
# inn i funksjonen
kom_model <- function(a_df) {
  lm(pm2 ~ fnr + Total_ya_p + inc_k1 + inc_k5 + uni_k_mf + uni_l_mf + Trade_pc_100K, 
     #data = pm2
     data = a_df
     )
}
...
```{r}
mod_sum <- pm2_n %>% 
  # unødvendig siden bare disse årene vi har
  # filter(aar %in% c("2008", "2009", "2010", "2011", "2012", 
...
coef_df <- mod_sum$model %>% 
  map_df(1) %>% 
  # vi trenger ikke endre til tibble, bare legge resultatet i en tibble
  tibble()
```
